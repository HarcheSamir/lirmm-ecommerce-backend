...
const KAFKA_TOPIC = 'review_events';
...
const sendMessage = async (type, payload) => {
    ...
    try {
        ...
        await producer.send({
            topic: KAFKA_TOPIC,
            messages: [{ key: payload.productId.toString(), value: JSON.stringify(message) }],
        });
        console.log(`KAFKA: Message sent to [${KAFKA_TOPIC}] | Type: [${type}] | Key: [${payload.productId}]`);
    } catch (error) {
