// Jenkinsfile (Corrected - Explicitly use default Kind Kubeconfig)
...
pipeline {
    ...
    environment {
        ...
        // --- Path for the Kubeconfig generated by Kind ---
        KIND_KUBECONFIG_FILE = "${WORKSPACE}/kind-kubeconfig-${env.BUILD_ID}.yaml"
    }
    ...
    stages {
        ...
        stage('Setup Temporary Kind Cluster') {
            steps {
                script {
                    ...
                    // Run kind create cluster WITHOUT '--kubeconfig' argument,
                    // let it update the default location temporarily if needed
                    sh "kind create cluster --name ${env.KIND_CLUSTER_NAME} --config ${env.KIND_CONFIG_FILE}"

                    // *** Generate the Kubeconfig file explicitly AFTER cluster is created ***
                    echo "Generating kubeconfig file for cluster ${env.KIND_CLUSTER_NAME}..."
                    // Use the standard command (not --internal) to get the host-accessible config
                    sh "kind get kubeconfig --name ${env.KIND_CLUSTER_NAME} > ${env.KIND_KUBECONFIG_FILE}"
                    echo "Kubeconfig saved to ${env.KIND_KUBECONFIG_FILE}"
                    // Sanity check the generated file
                    sh "echo '--- Generated Kubeconfig ---'; cat ${env.KIND_KUBECONFIG_FILE}; echo '-------------------------'"
        ...
        stage('Deploy Application to Kind') {
             steps {
                script {
                    echo "Using Specific Kubeconfig: ${env.KIND_KUBECONFIG_FILE}"

                    // *** ADDED: Verify kubectl connection using the generated Kubeconfig ***
                    echo "Verifying kubectl connection using generated config..."
                    sh "kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} cluster-info || (echo 'Initial cluster-info failed, retrying after delay...' && sleep 10 && kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} cluster-info)"
                    sh "kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} get nodes"
                    ...
                    // *** MODIFIED: Use generated Kubeconfig for apply ***
                    sh "kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} apply -f ${env.KUBERNETES_MANIFEST_RENDERED_FILE}"
                    ...
                    timeout(time: 15, unit: 'MINUTES') {
                        DEPLOYMENT_NAMES.each { deploymentName ->
                           ...
                           // *** MODIFIED: Use generated Kubeconfig for rollout status ***
                           sh "kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} rollout status deployment/${deploymentName} --watch=true --timeout=10m"
                        }
                    }
                    ...
                    echo "Checking Consul service health (informational)..."
                     try {
                        // *** MODIFIED: Use generated Kubeconfig for exec ***
                        sh """
                           kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} exec -i deployment/consul-deployment -c consul -- sh -c 'apk add --no-cache curl &> /dev/null || true'
                           kubectl --kubeconfig ${env.KIND_KUBECONFIG_FILE} exec -i deployment/consul-deployment -c consul -- curl -s http://localhost:8500/v1/health/state/any > consul_health.txt
                           echo '--- Consul Health ---'
    ...
    post {
        always {
            script {
                 ...
                 // *** ADDED: Remove the temporary Kind kubeconfig file ***
                 echo "Removing temporary Kind kubeconfig file: ${env.KIND_KUBECONFIG_FILE}"
                 sh "rm -f ${env.KIND_KUBECONFIG_FILE} || true"
