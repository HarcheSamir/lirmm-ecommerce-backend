...
const KAFKA_TOPIC = 'payment_events';
...
const sendMessage = async (type, payload) => {
    ...
    try {
        ...
        await producer.send({
            topic: KAFKA_TOPIC,
            messages: [{ key: payload.orderId.toString(), value: JSON.stringify(message) }],
        });
        console.log(`KAFKA: Message sent to topic [${KAFKA_TOPIC}] | Type: [${type}] | Key: [${payload.orderId}]`);
    } catch (error) {
